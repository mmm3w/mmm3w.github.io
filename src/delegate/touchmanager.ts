...
export class TouchManager {
    private _startY: number;            // 点击开始时的x值
    private _startX: number;            // 点击开始时的y值
    private _lastX: number;             // 单击时的x值
    private _lastY: number;             // 单击时的y值
    private _lastX1: number;            // 双击时第一个点的x值
    private _lastY1: number;            // 双击时第一个点的y值
    private _lastX2: number;            // 双击时第二个点的x值
    private _lastY2: number;            // 双击时第二个点的y值
    private _lastTouchDistance: number; // 两根以上手机触摸时指尖距离
    private _deltaX: number;            // 上次到现在x值移动的距离
    private _deltaY: number;            // 上次到现在y值移动的距离
    private _scale: number;             // 该帧放大率。放大操作以外时11。
    private _touchSingle: boolean;      // 单击时为true
    private _flipAvailable: boolean;    // filp是否有效
